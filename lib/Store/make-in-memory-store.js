...
const waLabelAssociationKey = (a1, a2) => (a1.type || '').localeCompare(a2.type || '')
    || (a1.chatId || '').localeCompare(a2.chatId || '')
    || (a1.labelId || '').localeCompare(a2.labelId || '');
...
/**
 * Classic in-memory store — keeps chats, contacts, messages, group metadata,
 * presences and labels in memory and stays in sync by binding to a socket's
 * event emitter. Drop-in replacement for the old Baileys `makeInMemoryStore`.
 */
export const makeInMemoryStore = (config = {}) => {
    ...
    /**
     * Binds to a Baileys event emitter — keeps the store in sync.
    ...
    const bind = (ev) => {
        ...
        ev.on('chats.update', (updates) => {
            for (let update of updates) {
                ...
                if (existing) {
                    // upsert keeps the dict in sync (and re-sorts on timestamp change)
    ...
    return {
        ...
        labelAssociations,
        bind,
        /** loads messages from the store, if not found and a socket is set, loads from the server */
...
// Minimal BufferJSON (de)serialization so binary fields survive a round-trip
function BufferJSON_replacer(_key, value) {
    if (value?.type === 'Buffer' && Array.isArray(value?.data)) {
        return value;
    }
    if (value instanceof Uint8Array || (typeof Buffer !== 'undefined' && Buffer.isBuffer(value))) {
        return { type: 'Buffer', data: Buffer.from(value).toString('base64'), __enc: 'base64' };
    }
    return value;
}

...
export class GroupCipher {
    ...
    async getPlainText(iv, key, ciphertext) {
        try {
            return aesDecryptWithIV(ciphertext, key, iv);
        }
        catch (e) {
...
// AES-256-CBC + PKCS7, ported from baron's whatsapp-rust-bridge fork
// (rb.aesDecryptWithIV / rb.aesEncrypWithIV). Implemented with node:crypto so no
// forked native dependency is required; behaviour is byte-identical to the
// previous libsignal/src/crypto.js path (both AES-256-CBC with default PKCS7).
const toBuf = v => (Buffer.isBuffer(v) ? v : Buffer.from(v.buffer ?? v, v.byteOffset ?? 0, v.byteLength ?? v.length));
function aesDecryptWithIV(buffer, key, iv) {
    const decipher = createDecipheriv('aes-256-cbc', toBuf(key), toBuf(iv));
    return Buffer.concat([decipher.update(toBuf(buffer)), decipher.final()]);
}
function aesEncryptWithIV(buffer, key, iv) {
    const cipher = createCipheriv('aes-256-cbc', toBuf(key), toBuf(iv));

...
export const useSqliteAuthState = async (pathOrFolder, options = {}) => {
  ...
  // Node's built-in SQLite (Node 22.5+). No native build, so it runs the same
  // on Termux, Linux servers, panels and Windows.
  let DatabaseSync;
  try {
    ({
      DatabaseSync
  ...
  // TRUNCATE (not WAL): WAL needs -wal/-shm shared memory that FAT/sdcardfs on
  // Android can't provide. TRUNCATE works on every filesystem.
  ...
  // node:sqlite has no db.transaction(); wrap manually.
  const runTx = fn => {
    db.exec('BEGIN');
    try {
      const r = fn();

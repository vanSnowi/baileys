...
import { areJidsSameUser, isHostedLidUser, isHostedPnUser, isJidBroadcast, isJidGroup, isJidNewsletter, isJidStatusBroadcast, isLidUser, isPnUser } from '../WABinary/index.js';
...
export function decodeMessageNode(stanza, meId, meLid) {
  ...
  if (!msgId) {
    throw new Boom('Invalid message stanza: missing id attribute', {
      data: stanza
    });
  }
  if (!from) {
    throw new Boom('Invalid message stanza: missing from attribute', {
      data: stanza
    });
  }
      ...
      if (Array.isArray(stanza.content)) {
        for (const {
          ...
          try {
            ...
            const msgToDecode = e2eType !== 'plaintext' ? unpadRandomMax16(msgBuffer) : msgBuffer;
            let msg = proto.Message.decode(msgToDecode);

        ...
        if (!pollCreation) {
          ...
          if (!pollEncKey) {
            ...
            if (lidCreator && lidVoter) {
              attempts.push({
                mode: 'lid',
                pollCreatorJid: lidCreator,
                voterJid: lidVoter
              });
            }
            attempts.push({
              mode: 'pn',
              pollCreatorJid: getKeyAuthor(creationMsgKey, meIdNormalised),
              voterJid: getKeyAuthor(message.key, meIdNormalised)
            });
            let voteMsg;
            let usedMode;
            ...
            if (voteMsg) {
              ...
              logger?.warn({
                creationMsgKey
              }, 'failed to decrypt poll vote (lid + pn)');
            }
          }
        }
      } catch (err) {

    ...
    if (creationMsgKey?.id && vote?.encPayload && vote?.encIv && typeof getMessage === 'function') {
      try {
        ...
        if (!pollCreation) {
          logger?.warn({
            creationMsgKey
          }, 'poll creation message not found, cannot decrypt vote');
        } else {
          const unwrapped = normalizeMessageContent(pollCreation);
          const pollEncKey = unwrapped?.messageContextInfo?.messageSecret || pollCreation?.messageContextInfo?.messageSecret;
          if (!pollEncKey) {
            logger?.warn({
              creationMsgKey
            }, 'poll vote: missing messageSecret');
          } else {
            const meIdNormalised = jidNormalizedUser(meId);
            ...
            const lidCreator = creationMsgKey.fromMe ? meLid : isLidUser(creatorKey) ? jidNormalizedUser(creatorKey) : undefined;
            const lidVoter = isFromMe ? meLid : participant && isLidUser(participant) ? jidNormalizedUser(participant) : isLidUser(remoteJid) ? jidNormalizedUser(remoteJid) : undefined;
            const pnCreator = getKeyAuthor(creationMsgKey, meIdNormalised);
            const pnVoter = getKeyAuthor(message.key, meIdNormalised);
            // WhatsApp may encrypt a poll vote with LID or PN for the creator
            // and the voter independently (e.g. LID self-chats), so lid+lid /
            // pn+pn alone can miss it. Try every distinct combination.
            const attempts = [];
            const seenAttempts = new Set();
            const pushAttempt = (mode, pollCreatorJid, voterJid) => {
              if (!pollCreatorJid || !voterJid) return;
              const dedup = `${pollCreatorJid}|${voterJid}`;
              if (seenAttempts.has(dedup)) return;
              seenAttempts.add(dedup);
              attempts.push({
                mode,
                pollCreatorJid,
                voterJid
            ...
            pushAttempt('lid', lidCreator, lidVoter);
            pushAttempt('pn', pnCreator, pnVoter);
            pushAttempt('lid-creator/pn-voter', lidCreator, pnVoter);
            pushAttempt('pn-creator/lid-voter', pnCreator, lidVoter);
            let voteMsg;
            let usedMode;
            ...
            if (voteMsg) {
              ...
              logger?.warn({
                creationMsgKey,
                triedModes: attempts.map(a => a.mode)

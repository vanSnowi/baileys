...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        // Decrypt incoming poll votes and re-emit as messages.update -> pollUpdates.
        // LID-first (modern WA Web / LID chats) with a legacy PN fallback; fully guarded so a
        // failure never breaks message processing.
        const update = content.pollUpdateMessage;
        const creationMsgKey = update?.pollCreationMessageKey;
        const vote = update?.vote;
        if (creationMsgKey?.id && vote?.encPayload && vote?.encIv && typeof getMessage === 'function') {
            try {
                ...
                if (!pollCreation) {
                    ...
                    // messageSecret may sit on the content or on a wrapped (viewOnce/ephemeral) inner message
                    const unwrapped = normalizeMessageContent(pollCreation);
                    const pollEncKey = unwrapped?.messageContextInfo?.messageSecret
                        || pollCreation?.messageContextInfo?.messageSecret;
                    if (!pollEncKey) {
                        logger?.warn({ creationMsgKey }, 'poll vote: missing messageSecret');
                    }
                    else {
                        const meIdNormalised = jidNormalizedUser(meId);
                        const meLid = creds.me?.lid ? jidNormalizedUser(creds.me.lid) : undefined;
                        const isFromMe = message.key?.fromMe === true;
                        const remoteJid = message.key?.remoteJid || creationMsgKey.remoteJid;
                        const participant = message.key?.participant || creationMsgKey.participant;
                        const creatorKey = creationMsgKey.participant || creationMsgKey.remoteJid;
                        // Attempt 1 — LID-aware jids
                        const lidCreator = creationMsgKey.fromMe
                            ? meLid
                            : (isLidUser(creatorKey) ? jidNormalizedUser(creatorKey) : undefined);
                        const lidVoter = isFromMe
                            ? meLid
                            : (participant && isLidUser(participant)
                                ? jidNormalizedUser(participant)
                                : (isLidUser(remoteJid) ? jidNormalizedUser(remoteJid) : undefined));
                        // Attempt 2 — legacy PN jids (getKeyAuthor)
                        const attempts = [];
                        if (lidCreator && lidVoter) {
                            attempts.push({ mode: 'lid', pollCreatorJid: lidCreator, voterJid: lidVoter });
                        }
                        attempts.push({ mode: 'pn', pollCreatorJid: getKeyAuthor(creationMsgKey, meIdNormalised), voterJid: getKeyAuthor(message.key, meIdNormalised) });
                        let voteMsg;
                        let usedMode;
                        for (const a of attempts) {
                            try {
                                voteMsg = decryptPollVote(vote, {
                                    pollEncKey,
                                    pollCreatorJid: a.pollCreatorJid,
                                ...
                                // wrong jid combination -> GCM auth fails -> try next
                            }
                        }
                        if (voteMsg) {
                            logger?.debug({ creationMsgKey, mode: usedMode }, 'decrypted poll vote');
                            ev.emit('messages.update', [
                                {
                                    key: creationMsgKey,
                                    update: {
                            ...
                            logger?.warn({ creationMsgKey }, 'failed to decrypt poll vote (lid + pn)');
                        }
                    }
                }
            }
            catch (err) {
                logger?.warn({ err: err?.message, creationMsgKey }, 'poll vote handling error');
            }
        }
    }

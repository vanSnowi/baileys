...
export const makeMessagesSocket = config => {
  ...
  const relayMessage = async (jid, message, {
    ...
    if ((isRecipientOnly || isSecret || protectedSend) && !isGroup && !isStatus) {
      // These modes deliberately narrow the device set (own devices, the
      // target's linked devices, or all but the target's main device). Tell
      // the server not to fan out either, so it does not deliver a copy to an
      // excluded device (which would show up there as "Waiting for this
      // message").
      additionalAttributes = {
        ...additionalAttributes,
        device_fanout: 'false'
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isGroupOrStatus && !isRetryResend) {
        ...
        for (const {
          ...
          // isSecret: only the target's main (primary, no device suffix) device
          if (isSecret && !(!isMe && !hasDevice)) continue;
          // protected: everything except the target's linked (secondary) devices

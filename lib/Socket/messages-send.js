...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, cstoken = true}) => {
        const meId = assertMeId(authState.creds);
        const meLid = authState.creds.me?.lid;
        const isRetryResend = Boolean(participant?.jid && participant?.count != null);
        // a `participant` target passed WITHOUT a retry `count` is an explicit
        // "recipient-only" send: fan out to the recipient's devices but skip all
        // of our own (self) devices. No participant → normal fanout to all devices.
        const isRecipientOnly = Boolean(participant?.jid && participant?.count == null);
...
for (const { user, jid } of devices) {
    ...
    // recipient-only send: skip ALL of our own devices, so only the
    ...
    allRecipients.push(jid);
            ...
            // cstoken (nct anti-spam fallback) — ON by default (relayMessage({ cstoken: false }) to disable).
            // Mirrors WA Web: attach ONLY on a 1:1 send that has NO tctoken (i.e. a cold contact),
            // as HMAC-SHA256(nctSalt, utf8("<user>@lid")). No-op unless salt is stored and recipient has a LID.
            if (cstoken && is1on1Send && !tcTokenBuffer?.length) {
                try {
                    ...
                    if (isLidUser(lidJid)) {
                        ...
                        if (nctSalt?.length) {
                            const cs = createHmac('sha256', Buffer.from(nctSalt))
                                .update(Buffer.from(lidJid, 'utf8'))
                                .digest();
...
while (inner && typeof inner === "object") {
  const entry = Object.entries(inner).find(([, v]) =>
    v &&
    typeof v === "object" &&
    (v.message || v.templateMessage || v.buttonsMessage || v.interactiveMessage)
  );

...
export const makeMessagesSocket = config => {
  ...
  const relayMessage = async (jid, message, {
    ...
    if (isRecipientOnly && !isGroup && !isStatus) {
      // Own devices are deliberately excluded from the fan-out. Tell the
      // server not to fan out either, so it does not sync a copy to our own
      // devices (which would show up there as "Waiting for this message").
      additionalAttributes = {
        ...additionalAttributes,
        device_fanout: 'false'
      };

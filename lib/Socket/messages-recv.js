...
export const makeMessagesRecvSocket = config => {
  ...
  const requestPlaceholderResend = async (messageKey, msgData) => {
    // ignoreOfflineMessages gate 2: never ask the phone to resend missed
    // messages (automatic PDO requests and scheduled phone requests).
  ...
  const handleMessage = async node => {
    ...
    try {
      ...
      await messageMutex.mutex(async () => {
        ...
        // ignoreOfflineMessages gate 3: offline-queue messages are decrypted
        // and acked (above) but not surfaced to messages.upsert, so the bot
        // only handles real-time (notify) messages. Offline calls and
        // offline notifications are intentionally left untouched.

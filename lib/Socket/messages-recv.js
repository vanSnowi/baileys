...
export const makeMessagesRecvSocket = config => {
  ...
  const processNode = async (type, node, identifier, exec) => {
    // ignoreOfflineMessages: drop anything delivered from the offline queue
    // (messages/receipts/etc. that piled up while disconnected) so the bot
    // does not act on stale messages after reconnecting. Ack so WhatsApp
    // treats them as handled and stops redelivering.

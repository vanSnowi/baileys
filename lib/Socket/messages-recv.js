...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === S_WHATSAPP_NET) {
            ...
            logger.debug({ node }, 'recv encrypt notification (identity change)');
        }
    };
    ...
    const handleMessage = async (node) => {
        // NOTE: msmsg (Meta AI Message Secret Messages) are now decrypted via the
        // normal decryptMessageNode path. decode-wa-message wraps msmsg decryption
        // in its own try/catch (failures degrade to a CIPHERTEXT stub instead of
        // crashing), so the previous "ignore msmsg" guard is no longer needed.
        let acked = false;
        try {

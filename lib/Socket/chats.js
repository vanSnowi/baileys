    ...
    const newAppStateChunkHandler = (isInitialSync) => {
        return {
            onMutation(mutation) {
                ...
                // Capture the NCT salt from app-state — used to derive the <cstoken> anti-spam
                // fallback token for cold 1:1 sends (mirrors WA Web's WAWebNctSalt).
                const nctSalt = mutation?.syncAction?.value?.nctSaltSyncAction?.salt;
                if (nctSalt?.length) {
                    authState.keys
                        .set({ 'nct-salt': { default: Buffer.from(nctSalt) } })
